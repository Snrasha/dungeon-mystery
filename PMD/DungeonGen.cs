...
namespace PMD
{
	...
	public class DungeonGen
	{
		...
		public DungeonGenerationInfo dungeonGenerationInfo;
		public FloorGenerationStatus statusData;
		DungeonRandom dungeonRand;
		GenerationConstants generationConstants;
		AdvancedGenerationSettings advancedGenerationSettings;
		List<int> grid_cell_start_x = new List<int>();
		List<int> grid_cell_start_y = new List<int>();

		DungeonGenerationCallback dungeonGenerationCallback;
		GenerationStepLevel generationCallbackFrequency;

		public delegate void DungeonGenerationCallback(
			GenerationStepLevel generation_step_level,
			GenerationType generation_type,
			Dungeon dungeon_data,
			DungeonGenerationInfo dungeon_generation_info,
			FloorGenerationStatus floor_generation_status,
			List<int> grid_cell_start_x,
			List<int> grid_cell_start_y
		);
 ...
 */
		void OnCompleteGenerationStep(GenerationStepLevel generation_step_level, GenerationType generation_type)
		{
			//Check if the event is at or below the threshold we've set for accepting callbacks
			if (generationCallbackFrequency >= generation_step_level && dungeonGenerationCallback!=null)
			{
		...
		void ResetFloor()
		{
			dungeonData.list_tiles = new TileDungeon[FLOOR_MAX_X][];

			//Reset Room Tiles
			for (int x = 0; x < FLOOR_MAX_X; x++)
			{
				dungeonData.list_tiles[x] = new TileDungeon[FLOOR_MAX_Y];

				for (int y = 0; y < FLOOR_MAX_Y; y++)
				{
					dungeonData.list_tiles[x][y] = new TileDungeon();
			...
			dungeonData.fixed_room_tiles = new TileDungeon[8][];

			//Reset Fixed Room Tiles
			for (int x = 0; x < 8; x++)
			{
				dungeonData.fixed_room_tiles[x] = new TileDungeon[8];

				for (int y = 0; y < 8; y++)
				{
					dungeonData.fixed_room_tiles[x][y] = new TileDungeon();
				}
			}
			...
			//Reset Traps
			dungeonData.active_traps = new List<double>(64);

			grid_cell_start_x.Clear();
			grid_cell_start_y.Clear();
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_RESET_FLOOR);
		}
         ...
         */
		(List<int> list_x, List<int> list_y) GetGridPositions(int grid_size_x, int grid_size_y)
		{
			int sum_x = 0, sum_y = 0;
			...
			{
				list_x.Add(sum_x);
				sum_x += (int)Math.Floor((double)FLOOR_MAX_X / grid_size_x); //This is our grid step size
			}

			for (int y = 0; y <= grid_size_y; y++)
			{
				list_y.Add(sum_y);
				sum_y += (int)Math.Floor((double)FLOOR_MAX_Y / grid_size_y);
			}
		...
		public GridCell[][] InitDungeonGrid(int grid_size_x, int grid_size_y)
		{
			GridCell[][] grid = new GridCell[15][];

			for (int x = 0; x < 15; x++)
			{
				grid[x] = new GridCell[15];
			...
			for (int x = 0; x < grid_size_x; x++)
			{
				for (int y = 0; y < grid_size_y; y++)
				{
					if (statusData.floor_size == FloorSize.FLOOR_SIZE_SMALL && x >= (grid_size_x / 2))
					{
						grid[x][y].is_invalid = true;
					}
					else if (statusData.floor_size == FloorSize.FLOOR_SIZE_MEDIUM && x >= (3 * grid_size_x) / 4)
					{
						grid[x][y].is_invalid = true;
					}
					else
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_INIT_DUNGEON_GRID);
		 ...
		 */
		bool IsCurrentMissionTypeExact(MissionSubtype type , MissionSubtype subtype)
		{
		 ...
		 */
		bool IsOutlawMonsterHouseFloor()
		{
			return IsCurrentMissionTypeExact(MissionSubtype.MISSION_ARREST_OUTLAW, MissionSubtype.MISSION_OUTLAW_MONSTER_HOUSE);
		}
		/**
 * NA: 02349748
 ...
 */
		bool FloorHasMissionMonster()
		{
			if (dungeonData.mission_destination.is_destination_floor)
			{
				if (
					dungeonData.mission_destination.mission_type == MissionSubtype.MISSION_RESCUE_CLIENT ||
					dungeonData.mission_destination.mission_type == MissionSubtype.MISSION_RESCUE_TARGET ||
					dungeonData.mission_destination.mission_type == MissionSubtype.MISSION_ESCORT_TO_TARGET ||
					dungeonData.mission_destination.mission_type == MissionSubtype.MISSION_DELIVER_ITEM ||
					dungeonData.mission_destination.mission_type == MissionSubtype.MISSION_SEARCH_FOR_TARGET ||
					dungeonData.mission_destination.mission_type == MissionSubtype.MISSION_TAKE_ITEM_FROM_OUTLAW ||
					dungeonData.mission_destination.mission_type == MissionSubtype.MISSION_ARREST_OUTLAW
				)
				{
					return true;
				}
		 ...
		 */
		bool IsDestinationFloorWithMonster()
		{
			return FloorHasMissionMonster();
		}

		public void AssignRooms(GridCell[][] grid, int grid_size_x, int grid_size_y, int number_of_rooms)
		{
			int extraRooms = dungeonRand.RandInt(3);
			...
			bool[] random_room_bits = new bool[256];
			...
			int max_rooms = grid_size_x * grid_size_y;

			for (int x = 0; x < 64; x++)
			{
				int a = dungeonRand.RandInt(max_rooms);
				int b = dungeonRand.RandInt(max_rooms);

				bool temp = random_room_bits[a];
			...
			for (int x = 0; x < grid_size_x; x++)
			{
				for (int y = 0; y < grid_size_y; y++)
				{
					...
					if (random_room_bits[counter])
					{
						...
						if (odd_x != 0 && y == 1 && x == (grid_size_x - 1) / 2)
						{
							grid[x][y].is_room = false;
						}
					}
		 ...
		 */
		public void GenerateStandardFloor(int grid_size_x, int grid_size_y, FloorProperties floor_props)
		{
			var (list_x, list_y) = GetGridPositions(grid_size_x, grid_size_y);

			 grid_cell_start_x = list_x;
			 grid_cell_start_y = list_y;

			GridCell[][] grid = InitDungeonGrid(grid_size_x, grid_size_y);
			...
			int cursor_x = UnityEngine.Random.Range(0,grid_size_x);
			int cursor_y = UnityEngine.Random.Range(0,grid_size_y);
		 ...
		 */
		public void GenerateOneRoomMonsterHouseFloor()
		{
			GridCell[][] grid = InitDungeonGrid(1, 1);
			...
			for (int x = grid[0][0].start_x; x < grid[0][0].end_x; x++)
			{
				for (int y = grid[0][0].start_y; y < grid[0][0].end_y; y++)
				{
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_ONE_ROOM_MONSTER_HOUSE_FLOOR);
		 ...
		 */
		void CreateRoomsAndAnchors(GridCell[][] grid, int grid_size_x, int grid_size_y, List<int> list_x, List<int> list_y, RoomFlags room_flags)
		{
			int room_number = 0;

			for (int y = 0; y < grid_size_y; y++)
			{
				int cur_val_y = list_y[y];
				int next_val_y = list_y[y + 1];

				for (int x = 0; x < grid_size_x; x++)
				{
					int cur_val_x = list_x[x];
					int next_val_x = list_x[x + 1];
					int range_x = next_val_x - cur_val_x - 4;
					int range_y = next_val_y - cur_val_y - 3;
					...
					if (!grid[x][y].is_room)
					{
						...
						int pt_x = dungeonRand.RandRange(cur_val_x + 2 + unk_x1, cur_val_x + 2 + range_x - unk_x2);
						int pt_y = dungeonRand.RandRange(cur_val_y + 2 + unk_y1, cur_val_y + 2 + range_y - unk_y2);
						...
						OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_CREATE_ANCHOR);
					}
					else
					{
						//This cell is a room!
						int room_size_x = dungeonRand.RandRange(5, range_x);
						int room_size_y = dungeonRand.RandRange(4, range_y);
						...
						if (room_size_x > Math.Floor((room_size_y * 3) / 2f))
						{
							room_size_x = (int)Math.Floor((room_size_y * 3) / 2f);
						}

						if (room_size_y > Math.Floor((room_size_x * 3) / 2f))
						{
							room_size_y = (int)Math.Floor((room_size_x * 3) / 2f);
						}

						int start_x = dungeonRand.RandInt(range_x - room_size_x) + cur_val_x + 2;
						int end_x = start_x + room_size_x;

						int start_y = dungeonRand.RandInt(range_y - room_size_y) + cur_val_y + 2;
						int end_y = start_y + room_size_y;
						...
						for (int room_x = start_x; room_x < end_x; room_x++)
						{
							for (int room_y = start_y; room_y < end_y; room_y++)
							{
						...
						bool flag_secondary = dungeonRand.RandInt(100) < generationConstants.secondary_structure_flag_chance;
						...
						bool flag_imp = room_flags.f_room_imperfections;
						...
						OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_CREATE_ROOM);
					}
				}
			}


			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_CREATE_ROOMS_AND_ANCHORS);
		}
		 ...
		 *
		 */
		void AssignGridCellConnections(GridCell[][] grid, int grid_size_x, int grid_size_y, int cursor_x, int cursor_y, FloorProperties floor_props
		)
		{
			...
			//	2: west
			// 	3: south
			CardinalDirection direction = (CardinalDirection)dungeonRand.RandInt(4);
			...
			for (int i = 0; i < floor_props.floor_connectivity; i++)
			{
				...
				int test = dungeonRand.RandInt(8);
				int new_direction = dungeonRand.RandInt(4);

				if (test < 4)
				{
					//Shuffle to a new direction
					direction = (CardinalDirection)new_direction;
				}
						...
						for (int x = 0; x < grid_size_x; x++)
						{
							if (!grid[x][y].is_invalid && !grid[x][y].is_room)
							{
								...
								if (count_connect == 1)
								{
									...
									direction = (CardinalDirection)dungeonRand.RandInt(4);
									bool ok = false;
									...
									//This section retains the original voidality
									else
									{
										if (direction == CardinalDirection.DIR_RIGHT && !grid[x + 1][y].is_invalid)
										{
		 ...
		 */
		void CreateHallway(int start_x, int start_y, int end_x, int end_y, bool vertical, int turn_x, int turn_y)
		{
			int cur_x = start_x;
			int cur_y = start_y;
			int counter = 0;
		 ...
		 *
		 */
		void CreateGridCellConnections(GridCell[][] grid, int grid_size_x, int grid_size_y, List<int> list_x, List<int> list_y, bool disable_room_merging)
		{
				...
				for (int y = 0; y < grid_size_y; y++)
				{
					...
					if (grid[x][y].should_connect_to_top)
					{
						...
						if (!grid[x][y - 1].is_invalid)
						{
							...
							OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_CREATE_HALLWAY);
						}
			...
			{
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_CREATE_GRID_CELL_CONNECTIONS);
				return;
			}
			...
			for (int x = 0; x < grid_size_x; x++)
			{
				for (int y = 0; y < grid_size_y; y++)
				{
					int chance = dungeonRand.RandInt(100);

					int src_x, src_y, dst_x, dst_y, merge_room_index;
					...
					{
						int chance_two = dungeonRand.RandInt(4);
						...
						)
						{
							...
							for (int cur_x = src_x; cur_x < dst_x; cur_x++)
							{
								for (int cur_y = src_y; cur_y < dst_y; cur_y++)
								{
									dungeonData.list_tiles[cur_x][cur_y].terrain_flags.terrain_type = TerrainType.TERRAIN_NORMAL;
							...
							OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_MERGE_ROOM);
						}
						else if (
						  chance_two == 1 &&
						  y >= 1 &&
		 ...
		 */
		void EnsureConnectedGrid(GridCell[][] grid, int grid_size_x, int grid_size_y, List<int> list_x, List<int> list_y)
		{
			//Flag for OnCompleteGenerationStep to verify if any changes were actually made
			bool was_grid_changed = false;
				...
				for (int y = 0; y < grid_size_y; y++)
				{
					...
					if (grid[x][y].is_room && !grid[x][y].has_secondary_structure)
					{
						...
						if (y > 0 && !grid[x][y - 1].is_invalid && !grid[x][y - 1].is_merged && grid[x][y - 1].is_connected)
						{
							...
							OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_ENSURE_CONNECTED_HALLWAY);
					...
					else
					{
						...
						OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_REMOVE_UNCONNECTED_ANCHOR);
						was_grid_changed = true;
					}
				}
			}
			...
			for (int x = 0; x < grid_size_x; x++)
			{
				for (int y = 0; y < grid_size_y; y++)
				{
					...
					for (int cur_x = grid[x][y].start_x; cur_x < grid[x][y].end_x; cur_x++)
					{
						for (int cur_y = grid[x][y].start_y; cur_y < grid[x][y].end_y; cur_y++)
						{
					...
					//Technicially this part of the void doesn't consider whether the grid cell is actually a room at all
					//(If it's an anchor, nothing visibly changes besides the loss of room index)
					if (grid[x][y].is_room)
					{
						OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_REMOVE_UNCONNECTED_ROOM);
					}
				}
			}
			...
			if (was_grid_changed)
			{
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_ENSURE_CONNECTED_GRID);
			}
		}
		 ...
		 */
		void SetTerrainObstacleChecked(TileDungeon tile, bool use_secondary_terrain, int room_index)
		{
		 ...
		 */
		void GenerateMazeLine(int x0, int y0, int xmin, int ymin, int xmax, int ymax, bool use_secondary_terrain, int room_index)
		{
			bool ok = true;
			while (ok)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					int pos_x = x0 + offset_x;
					int pos_y = y0 + offset_y;
		 ...
		 *
		 */
		void GenerateMaze(GridCell grid_cell, bool use_secondary_terrain)
		{
			grid_cell.is_maze_room = true;
			statusData.has_maze = true;

			int room_index = dungeonData.list_tiles[grid_cell.start_x][grid_cell.start_y].room_index;

			// Random walks from upper border
			for (int cur_x = grid_cell.start_x + 1; cur_x < grid_cell.end_x - 1; cur_x += 2)
			{
			...
			for (int cur_y = grid_cell.start_y + 1; cur_y < grid_cell.end_y - 1; cur_y += 2)
			{
			...
			for (int cur_x = grid_cell.start_x + 3; cur_x < grid_cell.end_x - 3; cur_x += 2)
			{
				for (int cur_y = grid_cell.start_y + 3; cur_y < grid_cell.end_y - 3; cur_y += 2)
				{
					if (dungeonData.list_tiles[cur_x][cur_y].terrain_flags.terrain_type == TerrainType.TERRAIN_NORMAL)
					{
		 ...
		 */
		void GenerateMazeRoom(GridCell[][] grid, int grid_size_x, int grid_size_y, int maze_chance)
		{
			...
			if (advancedGenerationSettings.allow_wall_maze_room_generation || dungeonGenerationInfo.floor_generation_attempts < 0)
			{
				...
				for (int i = 0; i < 64; i++)
				{
					int a = dungeonRand.RandInt(num_valid);
					int b = dungeonRand.RandInt(num_valid);
						...
						)
						{
							if ((grid[x][y].end_x - grid[x][y].start_x) % 2 != 0 && (grid[x][y].end_y - grid[x][y].start_y) % 2 != 0)
							{
								...
								if (values[counter])
								{
									...
									OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_GENERATE_MAZE_ROOM);
								}
		 ...
		 */
		void GenerateKecleonShop(GridCell[][] grid, int grid_size_x, int grid_size_y, int kecleon_chance)
		{
			...
			List<int> list_x = new List<int>(0xf);
			List<int> list_y = new List<int>(0xf);
			...
			for (int x = 0; x < 200; x++)
			{
				int a = dungeonRand.RandInt(0xf);
				int b = dungeonRand.RandInt(0xf);
			...
			for (int i = 0; i < list_x.Count; i++)
			{
				...
				for (int j = 0; j < list_y.Count; j++)
				{
					...
					for (int cur_x = statusData.kecleon_shop_min_x + 1; cur_x < statusData.kecleon_shop_max_x - 1; cur_x++)
					{
						for (int cur_y = statusData.kecleon_shop_min_y + 1; cur_y < statusData.kecleon_shop_max_y - 1; cur_y++)
						{
					...
					statusData.kecleon_shop_middle_x = (int)Math.Floor((statusData.kecleon_shop_min_x + statusData.kecleon_shop_max_x) / 2f);
					statusData.kecleon_shop_middle_y = (int)Math.Floor((statusData.kecleon_shop_min_y + statusData.kecleon_shop_max_y) / 2f);

					OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_GENERATE_KECLEON_SHOP);
		 ...
		 */
		void GenerateMonsterHouse(GridCell[][] grid, int grid_size_x, int grid_size_y, int monster_house_chance)
		{
				...
				for (int y = 0; y < grid_size_y; y++)
				{
					...
					)
					{
						if (values[counter])
						{
							...
							OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_GENERATE_MONSTER_HOUSE);
		 ...
		 */
		void GenerateExtraHallways(GridCell[][] grid, int grid_size_x, int grid_size_y, int num_extra_hallways)
		{
			//Flag for OnCompletedGenerationStep to verify if an extra hallway was added to the floor
			bool added_extra_hallway = false;

			for (int i = 0; i < num_extra_hallways; i++)
			{
				//Select a random grid cell
				int x = dungeonRand.RandInt(grid_size_x);
				int y = dungeonRand.RandInt(grid_size_y);
				...
				int cur_x = dungeonRand.RandRange(grid[x][y].start_x, grid[x][y].end_x);
				int cur_y = dungeonRand.RandRange(grid[x][y].start_y, grid[x][y].end_y);

				//Choose a random cardinal direction
				int direction = (dungeonRand.RandInt(4) * 2);

				//if invalid, rotate counter-clockwise until one works
				for (int j = 0; j < 3; j++)
				{
					if (direction == (int)DirectionId.DIR_DOWN && y >= grid_size_y - 1)
					{
						direction = (int)DirectionId.DIR_RIGHT;
					}

					if (direction == (int)DirectionId.DIR_RIGHT && x >= grid_size_x - 1)
					{
						direction = (int)DirectionId.DIR_UP;
					}

					if (direction == (int)DirectionId.DIR_UP && y <= 0)
					{
						direction = (int)DirectionId.DIR_LEFT;
					}

					if (direction == (int)DirectionId.DIR_LEFT && x <= 0)
					{
						direction = (int)DirectionId.DIR_DOWN;
					}
				}

				int room_index = dungeonData.list_tiles[cur_x][cur_y].room_index;
				...
				while (continue_walk)
				{
					if (dungeonData.list_tiles[cur_x][cur_y].room_index == room_index)
					{
						...
						cur_x += Directions.LIST_DIRECTIONS[(int)direction * 4];
						cur_y += Directions.LIST_DIRECTIONS[(int)direction * 4 + 2];
					}
					else
					{
						continue_walk = false;
				...
				bool valid = true;
				for (int n = cur_x - 2; n < cur_x + 3; n++)
				{
					for (int m = cur_y - 2; m < cur_y + 3; m++)
					{
						if (n < 0 || n >= FLOOR_MAX_X || m < 0 || m >= FLOOR_MAX_Y)
						{
							valid = false;
							break;
						}
				...
				check_x = cur_x + Directions.LIST_DIRECTIONS[check_direction * 4];
				check_y = cur_y + Directions.LIST_DIRECTIONS[check_direction * 4 + 2];
				...
				int steps = dungeonRand.RandInt(3) + 3;
				while (true)
				{
					...
					bool will_not_make_square = true;
					...
					if (steps == 0)
					{
						...
						int rotate_rand = dungeonRand.RandInt(100);
						...
						if (cur_x >= 32 && statusData.floor_size == FloorSize.FLOOR_SIZE_SMALL && direction == (int)DirectionId.DIR_RIGHT) break;
						if (cur_x >= 48 && statusData.floor_size == FloorSize.FLOOR_SIZE_MEDIUM && direction == (int)DirectionId.DIR_RIGHT) break;
					}

					//Move in the current direction
					cur_x += Directions.LIST_DIRECTIONS[direction * 4];
					cur_y += Directions.LIST_DIRECTIONS[direction * 4 + 2];
				}

				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_GENERATE_EXTRA_HALLWAY);
				added_extra_hallway = true;
			}

			if (added_extra_hallway)
			{
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_GENERATE_EXTRA_HALLWAYS);
			}
		}
		 ...
		 */
		void GenerateRoomImperfections(GridCell[][] grid, int grid_size_x, int grid_size_y)
		{
			//Flag for OnCompleteGenerationStep to verify if any imperfections were actually added
			bool added_room_imperfections = false;
			...
			for (int x = 0; x < grid_size_x; x++)
			{
				for (int y = 0; y < grid_size_y; y++)
				{
					...
					bool added_imperfections_to_this_room = false;

					int length = grid[x][y].end_x - grid[x][y].start_x + (grid[x][y].end_y - grid[x][y].start_y);

					length = (int)Math.Max(Math.Floor(length / 4f), 1);
					...
					for (int counter = 0; counter < length; counter++)
					{
						for (int i = 0; i < 2; i++)
						{
							...
							int starting_corner = dungeonRand.RandInt(4);
							int pt_x = 0,
								pt_y = 0,
								move_x = 0,
								move_y = 0;
							...
							for (int v = 0; v < 10; v++)
							{
								...
								if (dungeonData.list_tiles[pt_x][pt_y].terrain_flags.terrain_type == TerrainType.TERRAIN_NORMAL)
								{
									...
									int direction = (int)DirectionId.DIR_DOWN;
									while (direction < 8)
									{
										int next_x = pt_x + Directions.LIST_DIRECTIONS[direction * 4];
										int next_y = pt_y + Directions.LIST_DIRECTIONS[direction * 4 + 2];
									...
									if (direction == 8)
									{
										...
										int baseValue = starting_corner * 8;
										direction = 0;
										...
										while (direction < 8)
										{
											...
											if (CornerCardinalNeighborExpectOpen.CORNER_CARDINAL_NEIGHBOR_EXPECT_OPEN[baseValue + direction] != is_open) break;
					...
					{
						OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_GENERATE_ROOM_IMPERFECTION);
					}
				}
			}
			...
			{
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_GENERATE_ROOM_IMPERFECTIONS);
			}
		}
		 ...
		 */
		void SetSpawnFlag5(GridCell grid_cell)
		{
			for (int x = grid_cell.start_x; x < grid_cell.end_x; x++)
			{
				for (int y = grid_cell.start_y; y < grid_cell.end_y; y++)
				{
         ...
         */
		void GenerateSecondaryStructures(GridCell[][] grid, int grid_size_x, int grid_size_y)
		{
			//Flag for OnCompleteGenerationStep to verify if any secondary structures were generated
			bool generated_secondary_structure = false;
			...
			for (int y = 0; y < grid_size_y; y++)
			{
				for (int x = 0; x < grid_size_x; x++)
				{
					...
					SecondaryStructureType structure_type = (SecondaryStructureType)dungeonRand.RandInt(6); //0 = no secondary structure

					int room_size_x = grid[x][y].end_x - grid[x][y].start_x;
					int room_size_y = grid[x][y].end_y - grid[x][y].start_y;
					int middle_x = (int)Math.Floor((grid[x][y].end_x + grid[x][y].start_x) / 2f);
					int middle_y = (int)Math.Floor((grid[x][y].end_y + grid[x][y].start_y) / 2f);
					...
					if (structure_type == SecondaryStructureType.SECONDARY_STRUCTURE_MAZE_PLUS_DOT && statusData.secondary_structures_budget > 0)
					{
						...
						OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_GENERATE_SECONDARY_STRUCTURE);
					...
					else if (structure_type == SecondaryStructureType.SECONDARY_STRUCTURE_CHECKERBOARD && statusData.secondary_structures_budget > 0)
					{
						if (room_size_x % 2 != 0 && room_size_y % 2 != 0)
						{
							...
							for (int i = 0; i < 64; i++)
							{
								int rand_x = dungeonRand.RandInt(room_size_x);
								int rand_y = dungeonRand.RandInt(room_size_y);
					...
					else if (structure_type == SecondaryStructureType.SECONDARY_STRUCTURE_POOL)
					{
						if (room_size_x >= 5 && room_size_y >= 5)
						{
							...
							int rand_x1 = dungeonRand.RandRange(grid[x][y].start_x + 2, grid[x][y].end_x - 3);
							int rand_y1 = dungeonRand.RandRange(grid[x][y].start_y + 2, grid[x][y].end_y - 3);
							int rand_x2 = dungeonRand.RandRange(grid[x][y].start_x + 2, grid[x][y].end_x - 3);
							int rand_y2 = dungeonRand.RandRange(grid[x][y].start_y + 2, grid[x][y].end_y - 3);
							...
							if (statusData.secondary_structures_budget > 0)
							{
								...
								for (int cur_x = rand_x1; cur_x <= rand_x2; cur_x++)
								{
									for (int cur_y = rand_y1; cur_y <= rand_y2; cur_y++)
									{
										dungeonData.list_tiles[cur_x][cur_y].terrain_flags.terrain_type = TerrainType.TERRAIN_SECONDARY;
									}
								}
					...
					else if (structure_type == SecondaryStructureType.SECONDARY_STRUCTURE_DIVIDER && statusData.secondary_structures_budget > 0)
					{
						...
						if (dungeonRand.RandInt(2) == 0)
						{
							...
							for (int i = grid[x][y].start_x; i < grid[x][y].end_x; i++)
							{
						...
						else
						{
							...
							for (int i = grid[x][y].start_y; i < grid[x][y].end_y; i++)
							{
			...
			{
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_GENERATE_SECONDARY_STRUCTURES);
			}
		}
         ...
         */
		void GenerateOuterRingFloor(FloorProperties floor_props)
		{
			int grid_size_x = 6;
			int grid_size_y = 4;
			List<int> list_x = new List<int>() { 0, 5, 0x10, 0x1c, 0x27, 0x33, 0x38 };
			List<int> list_y = new List<int>() { 2, 7, 0x10, 0x19, 0x1e };
			...
			for (int y = 0; y < grid_size_y; y++)
			{
				for (int x = 0; x < grid_size_x; x++)
				{
					...
					{
						//Room
						int range_x = list_x[x + 1] - list_x[x] - 3;
						int range_y = list_y[y + 1] - list_y[y] - 3;

						int room_size_x = dungeonRand.RandRange(5, range_x);
						int room_size_y = dungeonRand.RandRange(4, range_y);
						int start_x = dungeonRand.RandInt(range_x - room_size_x) + list_x[x] + 2;
						int start_y = dungeonRand.RandInt(range_y - room_size_y) + list_y[y] + 2;
					...
					{
						//Hallway Anchor
						int start_x = dungeonRand.RandRange(list_x[x] + 1, list_x[x + 1] - 2);
						int start_y = dungeonRand.RandRange(list_y[y] + 1, list_y[y + 1] - 2);
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_OUTER_RING_FLOOR);
			...
			int cursor_x = dungeonRand.RandInt(grid_size_x);
			int cursor_y = dungeonRand.RandInt(grid_size_y);
         ...
         */
		void GenerateCrossroadsFloor(FloorProperties floor_props)
		{
			int grid_size_x = 5;
			int grid_size_y = 4;

			List<int> list_x = new List<int>() { 0, 0xb, 0x16, 0x21, 0x2c, 0x38 };
			List<int> list_y = new List<int>() { 1, 9, 0x10, 0x17, 0x1f };
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_CROSSROADS_FLOOR);
         ...
         */
		void GenerateTwoRoomsWithMonsterHouseFloor()
		{
			int grid_size_x = 2;
			int grid_size_y = 1;

			List<int> list_x = new List<int>() { 2, 0x1c, 0x36 };
			List<int> list_y = new List<int>() { 2, 0x1e };
			...
			for (int x = 0; x < grid_size_x; x++)
			{
				int range_x = list_x[x + 1] - list_x[x] - 3;
				int range_y = list_y[y + 1] - list_y[y] - 3;
				int room_size_x = dungeonRand.RandRange(10, range_x);
				int room_size_y = dungeonRand.RandRange(16, range_y);
				int start_x = dungeonRand.RandInt(range_x - room_size_x) + list_x[x] + 1;
				int start_y = dungeonRand.RandInt(range_y - room_size_y) + list_y[y] + 1;
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_TWO_ROOMS_WITH_MONSTER_HOUSE_FLOOR);
         ...
         */
		void GenerateLineFloor(FloorProperties floor_props)
		{
			int grid_size_x = 5;
			int grid_size_y = 1;

			List<int> list_x = new List<int>() { 0, 0xb, 0x16, 0x21, 0x2c, 0x38 };
			List<int> list_y = new List<int>() { 4, 0xf };
         ...
         */
		void GenerateCrossFloor(FloorProperties floor_props)
		{
			int grid_size_x = 3;
			int grid_size_y = 3;


			List<int> list_x = new List<int>() { 0xb, 0x16, 0x21, 0x2c };
			List<int> list_y = new List<int>() { 2, 0xb, 0x14, 0x1e };
         ...
         */
		void MergeRoomsVertically(int room_x, int room_y1, int room_dy, GridCell[][] grid)
		{
			int room_y2 = room_y1 + room_dy;

			int start_x = Math.Min(grid[room_x][room_y1].start_x, grid[room_x][room_y2].start_x);
			int end_x = Math.Max(grid[room_x][room_y1].end_x, grid[room_x][room_y2].end_x);
			int start_y = grid[room_x][room_y1].start_y;
			int end_y = grid[room_x][room_y2].end_y;

			//Carve out the new larger room, retaining the index of the first room
			int room_index = dungeonData.list_tiles[grid[room_x][room_y1].start_x][grid[room_x][room_y1].start_y].room_index;
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_MERGE_ROOM_VERTICALLY);
		}
         ...
         */
		void GenerateBeetleFloor(FloorProperties floor_props)
		{
			int grid_size_x = 3;
			int grid_size_y = 3;

			List<int> list_x = new List<int>() { 0x5, 0xf, 0x23, 0x32 };
			List<int> list_y = new List<int>() { 2, 0xb, 0x14, 0x1e };
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_MERGE_ROOM_VERTICALLY);
         ...
         */
		void GenerateOuterRoomsFloor(int grid_size_x, int grid_size_y, FloorProperties floor_props)
		{
			var (list_x, list_y) = GetGridPositions(grid_size_x, grid_size_y);
		...
		void ResetInnerBoundaryTileRows()
		{
			for (int x = 0; x < FLOOR_MAX_X; x++)
			{
				dungeonData.list_tiles[x][1] = new TileDungeon();
				...
				dungeonData.list_tiles[x][0x1e] = new TileDungeon();
         ...
         *
         * Due to the nature of how this void iterates left-to-right / top-to-bottom, by identifying junctions as any
         * open, non-hallway tile (room_index != 0xFF) adjacent to an open, hallway tile (room_index == 0xFF), the void
         ...
         * is a passable wall.
         */
		void SetSecondaryTerrainOnWall(TileDungeon tile)
		{
		...
		void GenerateSecondaryTerrainFormations(bool test_flag, FloorProperties floor_props)
		{
			...
			for (int i = 0; i < num_to_gen; i++)
			{
				...
				int steps_until_lake = dungeonRand.RandInt(50) + 10;
				...
				{
					//
					bool generated_river_tiles = false;

					//Fill in tiles in chunks of size 2-7 before changing the flow direction
					int num_tiles_fill = dungeonRand.RandInt(6) + 2;
					for (int v = 0; v < num_tiles_fill; v++)
					{
						if (pt_x >= 0 && pt_x < FLOOR_MAX_X)
						{
							...
							if (tile.terrain_flags.terrain_type == TerrainType.TERRAIN_SECONDARY)
							{
								...
								{
									OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_SECONDARY_TERRAIN_RIVER);
								}
								break;
							}
						...
						for (int j = 0; j < 64; j++)
						{
							...
							int offset_x = dungeonRand.RandInt(7) - 3;
							int offset_y = dungeonRand.RandInt(7) - 3;

							int target_x = pt_x + offset_x;
							int target_y = pt_y + offset_y;
						...
						OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_SECONDARY_TERRAIN_RIVER_LAKE);
					}
			...
			for (int i = 0; i < floor_props.secondary_terrain_density; i++)
			{
				...
				for (int v = 0; v < 80; v++)
				{
					...
					int x = dungeonRand.RandInt(8) + 1;
					int y = dungeonRand.RandInt(8) + 1;
				...
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_SECONDARY_TERRAIN_STANDALONE_LAKE);
			}
		 ...
		 void SpawnStairs(int x, int y, HiddenStairsType hidden_stairs_type)
		{
			...
			if (hidden_stairs_type == HiddenStairsType.HIDDEN_STAIRS_NONE && GetFloorType() == FloorType.FLOOR_TYPE_RESCUE)
			{
				...
				{
					for (int cur_y = 0; cur_y < FLOOR_MAX_Y; cur_y++)
					{
						if (
							dungeonData.list_tiles[cur_x][cur_y].terrain_flags.terrain_type == TerrainType.TERRAIN_NORMAL &&
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_SPAWN_STAIRS);
		}
         ...
         */
		void ShuffleSpawnPositions(List<int> spawn_x, List<int> spawn_y)
		{
			//Do twice as many swaps as there are items in the array
			for (int i = 0; i < spawn_x.Count * 2; i++)
			{
				int a = dungeonRand.RandInt(spawn_x.Count);
				int b = dungeonRand.RandInt(spawn_x.Count);

				int temp_x = spawn_x[a];
				int temp_y = spawn_y[a];
				spawn_x[a] = spawn_x[b];
				spawn_y[a] = spawn_y[b];
			...
			if (dungeonGenerationInfo.stairs_spawn_x == -1 || dungeonGenerationInfo.stairs_spawn_y == -1)
			{
				...
				for (int x = 0; x < FLOOR_MAX_X; x++)
				{
					for (int y = 0; y < FLOOR_MAX_Y; y++)
					{
						...
						{
							valid_spawns_x.Add(x);
							valid_spawns_y.Add(y);
						}
					}
				}

				if (valid_spawns_x.Count > 0)
				{
					//Randomly select one of the valid tiles to spawn the stairs on
					int stairs_index = dungeonRand.RandInt(valid_spawns_x.Count);
					...
					if (statusData.hidden_stairs_type != HiddenStairsType.HIDDEN_STAIRS_NONE)
					{
						...
						valid_spawns_x.RemoveAt(stairs_index);

						//valid_spawns_y.splice(stairs_index, 1);
						valid_spawns_y.RemoveAt(stairs_index);
						...
						if (dungeonData.floor + 1 < dungeonData.n_floors_plus_one)
						{
							...
							int hidden_index = dungeonRand.RandInt(valid_spawns_x.Count);
		...
		void SpawnNormalItems(FloorProperties floor_props, bool is_empty_monster_house)
        {
			...
			if (valid_spawns_x.Count > 0)
			{
				int num_items = floor_props.item_density;
				if (num_items != 0)
				{
				...
				if (num_items + 1 > 0)
				{
					...
					int cur_index = dungeonRand.RandInt(valid_spawns_x.Count);
					num_items += 1;
					for (int i = 0; i < num_items; i++)
					{
						int pos_x = valid_spawns_x[cur_index];
						int pos_y = valid_spawns_y[cur_index];

						cur_index++;

						if (cur_index == valid_spawns_x.Count)
						{
					...
					OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_SPAWN_ITEMS);
				}
			}
		...
		void SpawnBuriedItemsInWall(FloorProperties floor_props, bool is_empty_monster_house)
		{
			...
			if (valid_spawns_x.Count > 0)
			{
				int num_items = floor_props.buried_item_density;
		...
		void SpawnItemsTrapInMonsterHouse(FloorProperties floor_props, bool is_empty_monster_house)
        {
			...
			if (valid_spawns_x.Count > 0)
			{
				...
				int num_items = Math.Max(6, dungeonRand.RandRange((int)Math.Floor((5 * valid_spawns_x.Count) / 10f), (int)Math.Floor((8 * valid_spawns_x.Count) / 10f)));
		...
		void SpawnNormalTraps(FloorProperties floor_props, bool is_empty_monster_house)
        {
			...
			if (valid_spawns_x.Count > 0)
			{
				int num_traps = dungeonRand.RandRange((int)Math.Floor(floor_props.trap_density / 2f), floor_props.trap_density);
		...
		void SpawnPlayer(FloorProperties floor_props, bool is_empty_monster_house)
		{
			...
			if (dungeonGenerationInfo.player_spawn_x == -1 || dungeonGenerationInfo.player_spawn_y == -1)
			{
				...
				if (valid_spawns_x.Count > 0)
				{
					...
					int spawn_index = dungeonRand.RandInt(valid_spawns_x.Count);
					...
					OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_SPAWN_PLAYER);
				}
			}
			...
			void SpawnEnemies(FloorProperties floor_props, bool is_empty_monster_house)
		{
			...
			else
			{
				...
				num_enemies = dungeonRand.RandRange((int)Math.Floor(floor_props.enemy_density / 2f), floor_props.enemy_density);
			...
			if (valid_spawns_x.Count > 0 && num_enemies + 1 > 0)
			{
				//Randomly select among the valid item spawn spots
				ShuffleSpawnPositions(valid_spawns_x, valid_spawns_y);
				...
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_SPAWN_NON_MONSTER_HOUSE_ENEMIES);
			}

			if (!dungeonGenerationInfo.force_create_monster_house)
			{
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MAJOR, GenerationType.GEN_TYPE_SPAWN_ENEMIES);
				return;
			}
			...
			int num_monster_house_spawn = generationConstants.max_number_monster_house_enemy_spawns;
			...
			if (dungeonGenerationInfo.force_create_monster_house)
			{
				num_monster_house_spawn = (int)Math.Floor((num_monster_house_spawn * 3) / 2f);
			}
			...
			if (valid_spawns_x.Count > 0)
			{
				num_enemies = Math.Max(1, dungeonRand.RandRange((int)Math.Floor((7 * valid_spawns_x.Count) / 10f), (int)Math.Floor((8 * valid_spawns_x.Count) / 10f)));
				...
				OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_MINOR, GenerationType.GEN_TYPE_SPAWN_MONSTER_HOUSE_EXTRA_ENEMIES);
			}
         ...
         * iteration works))
         *
         * If any tile is walkable but wasn't reached, this void will return false.
         * If every tile was reached, this void will return true.
         */
		bool StairsAlwaysReachable(int x_stairs, int y_stairs, bool mark_unreachable)
		{
			StairsReachableFlags[][] test = new StairsReachableFlags[FLOOR_MAX_X][];

			for (int x = 0; x < FLOOR_MAX_X; x++)
			{
				test[x] = new StairsReachableFlags[FLOOR_MAX_Y];
		 ...
		 * 		- If the loop fails 10 times, defaults to generating a One-Room Monster House
		 */
		TileDungeon[][] GenerateFloor(FloorProperties floor_props)
		{
			...
			//value from fixed room data void call
			...
			for (spawn_attempts = 0; spawn_attempts < 10; spawn_attempts++)
			{
				...
				bool fixed_room = false;
				bool secondary_gen = false;
				...
				for (gen_attempts = 0; gen_attempts < 10; gen_attempts++)
				{
					...
					int grid_size_x = 2; //[r13, #+0x8]
					int grid_size_y = 2; //[r13, #+0x4]
					...
					if (Math.Floor(1f * FLOOR_MAX_X / grid_size_x) < 8)
					{
						grid_size_x = 1;
					}

					if (Math.Floor(1f * FLOOR_MAX_Y / grid_size_y) < 8)
					{
						grid_size_y = 1;
					}
				...
				bool is_empty_monster_house = dungeonRand.RandInt(100) < floor_props.itemless_monster_house_chance;
				...
				// No player or stair has been spawned
				if (floor_props.skip_entities)
				{
					break;
				}
			...
			OnCompleteGenerationStep(GenerationStepLevel.GEN_STEP_COMPLETE, GenerationType.GEN_TYPE_GENERATE_FLOOR);

			return dungeonData.list_tiles;
		}

		/**
         * GenerateDungeon - Entry void provided to generate a dungeon floor (not part of the original code)
         *
         ...
         * @param advanced_generation_settings - Modifications and patches to dungeon algorithm bugs / other advanced voidality
         * @param dungeon_generation_callback - A callback void to provide all information on the steps of generation as the algorithm progresses,
         * 										it will be called based on the the frequency specified in generation_callback_frequency
         * @param generation_callback_frequency - Determines how often to call the callback void. By default this will only be for the final generated layout,
         ...
         */
		public TileDungeon[][] GenerateDungeon(
		   FloorProperties floor_props,
					Dungeon dungeon_data,
					GenerationConstants generation_constants=null,
					AdvancedGenerationSettings advanced_generation_settings = null,
					DungeonGenerationCallback dungeon_generation_callback = null,
					GenerationStepLevel generation_callback_frequency = GenerationStepLevel.GEN_STEP_NONE)
		{
			...
			string json = JsonUtility.ToJson(dungeon_data, false);
			
			dungeonData = JsonUtility.FromJson<Dungeon>(json);
			...
			if (advanced_generation_settings != null)
			{
			...
			if (dungeon_generation_callback != null)
			{
				dungeonGenerationCallback = dungeon_generation_callback;
			}

			if (generation_callback_frequency != GenerationStepLevel.GEN_STEP_NONE)
			{
